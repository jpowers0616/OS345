...
// jurassic task
// ***********************************************************************
int dinoTask(int argc, char* argv[])
{
	int choice = 0; SWAP;
	int r = 0; SWAP;
	Semaphore* hunger = createSemaphore("hunger", BINARY, 0); SWAP;

	while (1)
	{
		SEM_WAIT(carMoving); SWAP;
		SEM_WAIT(deltaMutex); SWAP;
		{
			r = (rand() % 40) + 1; SWAP;
			insertDeltaClock(r, hunger); SWAP;
		}
		SEM_SIGNAL(deltaMutex); SWAP;
		SEM_WAIT(hunger); SWAP;
		choice = rand() % 2; SWAP;
		if (choice == 1)
		{
			SEM_WAIT(parkMutex); SWAP;
			{
				for (int i = 0; i < 4; i++)
				{
					if (myPark.cars[i].passengers == 3)
					{
						myPark.cars[i].passengers--; SWAP;
...
{
	char buf[32]; SWAP;
	int i = atoi(argv[0]); SWAP;
	sprintf(buf, "Car%d", i); SWAP;
	Semaphore* carRide[3]; SWAP;
	Semaphore* driverDone; SWAP;
	while (1)
	{
		///////////////////////////////////////////
		SEM_WAIT(fillSeat[i]); SWAP;
		{
		...
		SEM_SIGNAL(seatFilled[i]); SWAP;
		SEM_SIGNAL(carMoving); SWAP;

		/////////////////////////////////////////////////
		if (carRide[0] == carRide[1])
		{
			printf("car 0 == car 1");
			system("pause");
		}
		else if (carRide[1] == carRide[2])
		{
			printf("car 1 == car 2");
			system("pause");
		}
		else if (carRide[2] == carRide[0])
		{
			printf("car 2 == car 0");
			system("pause");
		}
		////////////////////////////////////////////////
		SEM_WAIT(rideOver[i]); SWAP;
		
		SEM_WAIT(parkMutex); SWAP;
		{
			if (myPark.cars[i].passengers < 3)
			{
				SEM_SIGNAL(riderDead); SWAP;
			}
		}
		SEM_SIGNAL(parkMutex);

		SEM_SIGNAL(carRide[0]); SWAP;
		SEM_SIGNAL(carRide[1]); SWAP;
		SEM_SIGNAL(carRide[2]); SWAP;
...
int driverTask(int argc, char* argv[])
{
	...
	Semaphore* driverDone; SWAP;
	int i = atoi(argv[0]); SWAP;
	sprintf(buf, "Starting driverDone%d", i); SWAP;
	driverDone = createSemaphore(buf, BINARY, 0); SWAP;
	sprintf(buf, "findingGift%d", i); SWAP;
	int r = 0; SWAP;
	Semaphore* findingGift = createSemaphore(buf, BINARY, 0); SWAP;
	int task;
	...
	while (1)
	{
		...
		{
			SEM_SIGNAL(gotit); SWAP;
			SEM_SIGNAL(driverReady); SWAP;
			SEM_WAIT(mailboxMutex); SWAP;
			{
				mail = driverDone; SWAP;
				SEM_SIGNAL(mailboxReady); SWAP;
				SEM_WAIT(mailAcquired); SWAP;
			}
			SEM_SIGNAL(mailboxMutex); SWAP;
		...
		else if (semTryLock(needTicket))
		{
			...
			SEM_WAIT(parkMutex); SWAP;
			{
				myPark.drivers[i] = -1; SWAP;
			}
			SEM_SIGNAL(parkMutex); SWAP;
			SEM_WAIT(deltaMutex); SWAP;
			{
				r = (rand() % 10) + 1; SWAP;
				insertDeltaClock(r, findingGift); SWAP;
			}
			SEM_SIGNAL(deltaMutex); SWAP;
			SEM_WAIT(findingGift); SWAP;
			SEM_SIGNAL(takeTicket); SWAP;
			SEM_WAIT(buyTicket); SWAP;
		}
		else if (semTryLock(needGift))
		{
			SEM_SIGNAL(gotit); SWAP;
			SEM_WAIT(parkMutex); SWAP;
			{
				myPark.drivers[i] = 5; SWAP;
			}
			SEM_SIGNAL(parkMutex); SWAP;
			SEM_WAIT(deltaMutex); SWAP;
			{
			...
			SEM_SIGNAL(deltaMutex); SWAP;
			SEM_WAIT(findingGift); SWAP;
			SEM_SIGNAL(getGift); SWAP;
			SEM_WAIT(buyGift); SWAP;
		}
		else break;
	}
	return 0;
...
int visitorTask(int argc, char* argv[])
{
	int i = atoi(argv[0]); SWAP;
	char buf[32]; SWAP;
	sprintf(buf, "wait%d", i); SWAP;
	Semaphore* wait = createSemaphore(buf, BINARY, 0); SWAP;
	sprintf(buf, "carRide%d", i); SWAP;
	Semaphore* carRide = createSemaphore(buf, BINARY, 0); SWAP;
	int r = 0; SWAP;
	...
	{
		r = (rand() % 30) + 1; SWAP;
		insertDeltaClock(r, wait); SWAP;
	}
	SEM_SIGNAL(deltaMutex); SWAP;
	SEM_WAIT(wait); SWAP;*/
	/////////////////////////////////////////

	SEM_WAIT(inPark); SWAP;
	{
		//////////////////////////////////////////////
		SEM_WAIT(parkMutex); SWAP;
		{
		...
		SEM_SIGNAL(mailboxMutex); SWAP;
		SEM_WAIT(carRide); SWAP;

		if (!semTryLock(riderDead))
		{
			SEM_WAIT(parkMutex); SWAP;
			{
				myPark.numTicketsAvailable++; SWAP;
			...
			SEM_WAIT(inGiftshop); SWAP;
			{
				...
				SEM_WAIT(driverMutex); SWAP;
				{
					...
					SEM_SIGNAL(wakeUpDriver); SWAP;
					SEM_WAIT(gotit); SWAP;
					SEM_WAIT(getGift); SWAP;
					SEM_SIGNAL(buyGift); SWAP;
				}
				SEM_SIGNAL(driverMutex); SWAP;
			}
...
int jurassicTask(int argc, char* argv[])
{
	...
	deltaMutex = createSemaphore("deltaMutex", BINARY, 1); SWAP;
	mailboxMutex = createSemaphore("mailboxMutex", BINARY, 1); SWAP;
	mailboxReady = createSemaphore("mailReady", BINARY, 0); SWAP;
	mailAcquired = createSemaphore("mailAcquired", BINARY, 0); SWAP;

	tickets = createSemaphore("tickets", COUNTING, MAX_TICKETS); SWAP;
	visitors = createSemaphore("visitors", COUNTING, NUM_VISITORS); SWAP;
	inPark = createSemaphore("inPark", COUNTING, MAX_IN_PARK); SWAP;
	...
	needGift = createSemaphore("needGift", BINARY, 0); SWAP;
	wakeUpDriver = createSemaphore("wakeUpDriver", BINARY, 0); SWAP;
	gotit = createSemaphore("gotit", BINARY, 0); SWAP;
	carMoving = createSemaphore("carMoving", BINARY, 0); SWAP;
	riderDead = createSemaphore("riderDead", BINARY, 0); SWAP;

	
	getPassenger = createSemaphore("getPassenger", BINARY, 0); SWAP;	
	driverReady = createSemaphore("driverReady", BINARY, 0); SWAP;
	takeTicket = ticketReady = createSemaphore("takeTicket", BINARY, 0); SWAP;
	buyTicket = createSemaphore("buyTicket", BINARY, 0); SWAP;
	getGift = createSemaphore("getGift", BINARY, 0); SWAP;
	buyGift = createSemaphore("buyGift", BINARY, 0); SWAP;
	...
	} while (myPark.numExitedPark < (NUM_VISITORS - myPark.numEaten));
...
DC* deltaClock[MAX_IN_PARK * 2];
int countDC = 0;

int insertDeltaClock(int time, Semaphore* sem)
{
	DC* insert = (DC*)malloc(sizeof(DC));
	insert->t = time;
	insert->s = sem;
	if (countDC == 0)
	{
	...
	{
		int index = 0;
		while ((deltaClock[index] != NULL) && (time >= deltaClock[index]->t))
		{
			time = time - deltaClock[index]->t;
			index++;
		}
		if (index == countDC)
		{
			deltaClock[index] = insert;
			countDC++;
		}
		else
		{
			deltaClock[index]->t = deltaClock[index]->t - time;
			for (int i = countDC; i > index; i--)
			{
...
int poll_removeDC()
{
	if (deltaClock[0] == NULL) return 0;
	DC* remove;
	while ((deltaClock[0] != NULL) && (deltaClock[0]->t == 0))
	{
		SEM_SIGNAL(deltaClock[0]->s);
		remove = deltaClock[0];
