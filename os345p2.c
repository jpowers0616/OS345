...
// ?? 3) If blocked, indicate which semaphore
	...
	{
		Semaphore s = *semaphoreList->semLink;
		while (s.semLink != NULL)
		{
			printf("\n** %s Blocked Queue **", s.name);
			printQ(&s.blockedQ);
			s = *s.semLink;
		}
	}
...
{
	char* curTime = (char*)malloc(sizeof(char) * 20);
	while (1)
	{
		printf("\nCurrent task# %d, Time: %s", curTask, myTime(curTime));
		SEM_WAIT(tics10sec);
	}
	return 0;
}
