...
{
	int found, newArgc;					// # of arguments
	char** newArgv;						// pointers to arguments
	int(*functionPtr)(char* name, int(*task)(int, char**), int priority, int argc, char* argv[]);
	bool AmpTask;
	...
	while (1)
	{
...

		{
			...
			{
				inBufferPrev[i] = inBuffer[i];
			}
			newArgv = (char**)malloc(sizeof(char*) * (MAX_ARGS + 1));
			newArgc = 0;

			unsigned int k, j, prev;
			k = 0; prev = 0;
			do
			{
				bool quotes = FALSE;
				...
				if (inBuffer[k] == ' ' || inBuffer[k] == NULL || inBuffer[k] == '&')
				{
					char* _command = (char*)malloc(sizeof(char) * ((k - prev) + 1));
					int j = 0;
					for (int i = prev; i < k; i++)
					{
						if (quotes)
						{
							_command[j] = inBuffer[i];
						}
						else
						{
							_command[j] = tolower(inBuffer[i]);
						}
						j++;
					}
					_command[j] = NULL;

					newArgv[newArgc] = _command;
					newArgc++; prev = k + 1;
				}

			} while (inBuffer[k++] != NULL);
			
			//check whether command need to become a new task
			if (inBuffer[k - 2] == '&')
			{
				AmpTask = TRUE;
				newArgc--;
		...
		// look for command
		for (int i = found = 0; i < NUM_COMMANDS; i++)
		{
				 ...
				 !strcmp(newArgv[0], commands[i]->shortcut))
			{
				...
				if (AmpTask)
				{
					functionPtr = &createTask;
					int taksID = (*functionPtr)(newArgv[0], commands[i]->func, MED_PRIORITY, newArgc, newArgv);
				}
				else
				{
		...
		for (int i=0; i < INBUF_SIZE; i++) inBuffer[i] = 0;
	}
...
int P1_Add(int argc, char* argv[])
{
	...
	for (int i = 1; i < argc; i++)
	{
		if (argv[i][1] == 'x')
		{
			int x = (int)strtol(argv[i], NULL, 0);
			sum += x;
		}
		else
		{
...
int P1_Calculator(int argc, char* argv[])
{
	strcpy(argv[0], "0");
	int a = atoi(argv[1]);
	int b = atoi(argv[3]);
...
Command** P1_init()
{
	...
	commands[i++] = newCommand("add", "ad", P1_Add, "Add all numbers on command line");
	commands[i++] = newCommand("args", "ag", P1_Args, "Display remaining command line args");
	commands[i++] = newCommand("calculator", "calc", P1_Calculator, "Open Calulator");
